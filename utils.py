...
# -----------------------------------------------------------------------------
# Logistic Regression Pipeline for Spam Classification with Sample Weights
# -----------------------------------------------------------------------------
def train_logit(X: pd.DataFrame, y: pd.Series) -> Pipeline:
    """
    Build a machine learning pipeline that preprocesses the spam data and trains a logistic regression model
    using sample weights to account for class imbalance.
    ...
    """
    numeric_features = ["num_links", "num_exclamations", "email_length"]
    categorical_features = ["sender_domain", "email_client"]
    ordered_features = ["suspicion_score", "urgency_level"]
    text_features = ["subject", "email_body"]

    numeric_transformer = StandardScaler()
    categorical_transformer = OneHotEncoder(handle_unknown="ignore")

    # Text transformer: Combine the two text columns and convert them into TF-IDF features.
    text_pipeline = Pipeline(
        [
            (
                "combine",
                FunctionTransformer(
                    lambda x: x.iloc[:, 0] + " " + x.iloc[:, 1], validate=False
                ),
            ),
    ...
    preprocessor = ColumnTransformer(
        transformers=[
            ...
            (
                ...
                ordered_features,
            ),  # Treat ordinal features as categorical
    ...
    est_intercept = pipeline.named_steps["classifier"].intercept_[0]
    est_beta = pipeline.named_steps["classifier"].coef_[0]

    # print("\n--- Beta Coefficients ---")
    # print("Estimated Intercept:", est_intercept)
    # print("Estimated Coefficients:", est_beta)
